...
## 4. 분석 및 결론
1.  **실험 1: 원본 데이터 K-NN (특성 4개)**
    * main.py 스크립트 실행 결과, 4개의 특성을 모두 사용한 K-NN 모델은 K=1일 때부터 0.9778(97.8%)의 매우 높은 테스트 정확도를 보였습니다.

2.  **실험 2: PCA 적용 데이터 K-NN (주성분 2개)**
    * PCA(n_components=2)를 적용하여 특성을 2개로 축소한 데이터는 K=1일 때 0.8889(약 88.9%)의 정확도를 보였습니다.
    * K=3, 5, 7 등 홀수 값에서 0.9556(약 95.6%)으로 가장 높은 정확도를 기록했습니다.
3.  **결론 (해석 요약)**
    * 성능 비교: 원본 데이터를 사용했을 때(정확도 100%)가 PCA로 차원을 축소했을 때(최대 95.6%)보다 성능이 더 높았습니다.
    * 해석: 4개의 특성을 2개로 줄이는 과정에서 일부 정보가 손실되어 정확도가 소폭 하락했습니다.
    * 의의: 그럼에도 불구하고, 단 2개의 주성분(PC1, PC2)만으로도 95.6%라는 높은 분류 정확도를 달성할 수 있었습니다. 이는 붓꽃 데이터의 경우, 2개의 주성분만으로도 품종을 구분하는 대부분의 정보를 설명할 수 있음을 의미합니다.
